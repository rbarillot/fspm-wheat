...
def main(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
         ...
         external_soil_model=False, N_fertilizations=None, PLANT_DENSITY=None, update_parameters_all_models=None,
    ...
    nitrates_uptake_data_filepath = os.path.join(INPUTS_DIRPATH, NITRATES_UPTAKE_FORCINGS_FILENAME)
    nitrates_uptake_data_df = pd.read_csv(nitrates_uptake_data_filepath)
    nitrates_uptake_data_grouped = nitrates_uptake_data_df.groupby(cnwheat_simulation.Simulation.ORGANS_T_INDEXES)
    ...
    def force_nitrates_uptake(t, population, nitrates_uptake_df, g):
        ...
        for plant in population.plants:
            ...
            for axis in plant.axes:
                ...
                group = nitrates_uptake_df.get_group((t, plant.index, axis.label, 'roots'))
                nitrates_uptake_data_to_use = group.loc[group.first_valid_index(), group.columns.intersection(['Uptake_Nitrates'])].dropna().to_dict()
    ...
    force_nitrates_uptake(0, cnwheat_facade_.population, nitrates_uptake_data_grouped, g)
    ...
    if run_simu:
        ...
        try:
                        ...
                        for t_elongwheat in range(t_farquharwheat, t_farquharwheat + FARQUHARWHEAT_TIMESTEP, ELONGWHEAT_TIMESTEP):
                            ...
                            for t_growthwheat in range(t_elongwheat, t_elongwheat + ELONGWHEAT_TIMESTEP, GROWTHWHEAT_TIMESTEP):
                                ...
                                for t_cnwheat in range(t_growthwheat, t_growthwheat + GROWTHWHEAT_TIMESTEP, CNWHEAT_TIMESTEP):
                                    ...
                                    if not external_soil_model and N_fertilizations is not None and len(N_fertilizations) > 0:
                                        if t_cnwheat in N_fertilizations.keys():
                                            cnwheat_facade_.soils[(1, 'MS')].nitrates += N_fertilizations[t_cnwheat]

                                    if t_cnwheat > 0:
                                        # Force the nitrates uptake of roots
                                        force_nitrates_uptake(t_cnwheat, cnwheat_facade_.population, nitrates_uptake_data_grouped, g)
                                        # run CNWheat
            ...
            for outputs_df_list, outputs_filename, index_columns in ((axes_all_data_list, AXES_OUTPUTS_FILENAME, AXES_INDEX_COLUMNS),
                ...
                if run_from_outputs and (outputs_filename in previous_outputs_dataframes.keys()):
    ...
    if generate_graphs:
        ...
        # - RER parameters
        rer_param = dict((k, v) for k, v in elongwheat_parameters.PARAMETERS.RERmax.items())
...
if __name__ == '__main__':
    ...
    main(2500, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False,
         ...
         heterogeneous_canopy=True, external_soil_model=True, N_fertilizations={1440: 357143, 2520: 1000000},
         PLANT_DENSITY={1: 250}, update_parameters_all_models=RERmax_vegetative_stages_example,
