...
def main(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
    ...
    if run_from_outputs:
        ...
        if forced_start_time > 0:
            ...
            last_t_step = max(previous_axes_outputs_dataframe['t'])
    ...
    # Name of the CSV files which contains the management data
    management = pd.read_csv(os.path.join(INPUTS_DIRPATH, MANAGEMENT_FILENAME), header=None, index_col=0, squeeze=True)
    ...
    if run_simu:
        ...
        try:
            ...
            for t_caribu in range(START_TIME, SIMULATION_LENGTH, SENESCWHEAT_TIMESTEP):
                ...
                if (t_caribu % CARIBU_TIMESTEP == 0) and (PARi_next_hours > 0) and bool(g.property('geometry')):
                    run_caribu = True
                else:
                    run_caribu = False
    ...
    if run_postprocessing:
        ...
        for postprocessing_file_basename, postprocessing_filename, index_columns in ((axes_postprocessing_file_basename, AXES_POSTPROCESSING_FILENAME, AXES_INDEX_COLUMNS),
                                                                                     (hiddenzones_postprocessing_file_basename, HIDDENZONES_POSTPROCESSING_FILENAME, HIDDENZONES_INDEX_COLUMNS),
                                                                                     (organs_postprocessing_file_basename, ORGANS_POSTPROCESSING_FILENAME, ORGANS_INDEX_COLUMNS),
                                                                                     (elements_postprocessing_file_basename, ELEMENTS_POSTPROCESSING_FILENAME, ELEMENTS_INDEX_COLUMNS),
                                                                                     (soils_postprocessing_file_basename, SOILS_POSTPROCESSING_FILENAME, SOILS_INDEX_COLUMNS)):
            postprocessing_filepath = os.path.join(POSTPROCESSING_DIRPATH, postprocessing_filename)
            postprocessing_df = postprocessing_df_dict[postprocessing_file_basename]
...
if __name__ == '__main__':
    main(3000, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False,
         show_3Dplant=False,
